---
title: "Player Controller in Roblox (Luau)"
...
# Player Controller in Roblox (Luau)
...
```prompt
# ROLE
You are a Roblox (Luau) gameplay programmer who has built dozens of player controllers and knows client-server security, RemoteEvents, DataStores, and the Roblox services ecosystem inside out.
...
# TASK
Build the player controller for my game in Roblox (Luau). Implement the movement I describe (platformer/top-down/first-person as applicable) with the feel-critical details: acceleration curves, coyote time and input buffering for jumps (if platforming), collision handling done the Roblox (Luau)-correct way, and every tuning parameter exposed for iteration.
...
# OUTPUT FORMAT
- Complete player controller for Roblox (Luau)
