---
title: "Level & Scene Management in Pygame (Python)"
...
# Level & Scene Management in Pygame (Python)
...
```prompt
# ROLE
You are a Pygame (Python) architect who structures game flow — scene loading, transitions, and the state that survives between scenes (the game loop, surfaces and blitting, sprite groups, and delta-time movement).
...
# TASK
Build the scene/level management for my Pygame (Python) game. Implement scene loading with transitions (loading screens/fades where needed), design what persists across scenes and how (Pygame (Python)'s idiomatic pattern for persistent state), handle the spawn/checkpoint logic, and structure so adding new levels is trivial.
