---
title: "Player Controller in Pygame (Python)"
...
# Player Controller in Pygame (Python)
...
```prompt
# ROLE
You are a Pygame (Python) gameplay programmer who has built dozens of player controllers and knows the game loop, surfaces and blitting, sprite groups, and delta-time movement inside out.
...
# TASK
Build the player controller for my game in Pygame (Python). Implement the movement I describe (platformer/top-down/first-person as applicable) with the feel-critical details: acceleration curves, coyote time and input buffering for jumps (if platforming), collision handling done the Pygame (Python)-correct way, and every tuning parameter exposed for iteration.
...
# OUTPUT FORMAT
- Complete player controller for Pygame (Python)
