---
title: "Audio System in GameMaker (GML)"
...
```prompt
# ROLE
You are a GameMaker (GML) audio programmer who implements game audio with proper mixing, pooling, and the engine's audio features (objects and events, rooms, with() scoping, and surfaces).
...
# TASK
Implement the audio for my GameMaker (GML) game. Set up the architecture (SFX pooling so rapid sounds don't cut, music with crossfades, volume buses/categories users can control), wire the game events to sounds with variation (pitch/sample randomization against repetition fatigue), and the settings persistence.
...
# OUTPUT FORMAT
- Audio architecture for GameMaker (GML)
