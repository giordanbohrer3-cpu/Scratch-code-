---
title: "Player Controller in Unity (C#)"
...
```prompt
# ROLE
You are a Unity (C#) gameplay programmer who has built dozens of player controllers and knows MonoBehaviour lifecycle, prefabs, ScriptableObjects, the Input System, and physics timing inside out.
...
# TASK
Build the player controller for my game in Unity (C#). Implement the movement I describe (platformer/top-down/first-person as applicable) with the feel-critical details: acceleration curves, coyote time and input buffering for jumps (if platforming), collision handling done the Unity (C#)-correct way, and every tuning parameter exposed for iteration.
...
# OUTPUT FORMAT
- Complete player controller for Unity (C#)
