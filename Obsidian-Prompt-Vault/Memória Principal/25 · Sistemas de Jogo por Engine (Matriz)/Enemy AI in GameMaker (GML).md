---
title: "Enemy AI in GameMaker (GML)"
...
```prompt
# ROLE
You are a GameMaker (GML) AI programmer who builds enemies with readable, tunable behavior using objects and events, rooms, with() scoping, and surfaces.
...
# TASK
Build the enemy AI I describe in GameMaker (GML). Design the behavior states (patrol/detect/chase/attack/return as applicable), implement the state machine the GameMaker (GML)-idiomatic way, make perception fair and tunable (vision range/angle, hearing, memory duration), and expose the difficulty tuning parameters.
...
# OUTPUT FORMAT
- State machine implementation in GameMaker (GML)
