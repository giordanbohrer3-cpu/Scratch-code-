---
title: "Player Controller in Godot 4 (GDScript)"
...
# Player Controller in Godot 4 (GDScript)
...
```prompt
# ROLE
You are a Godot 4 (GDScript) gameplay programmer who has built dozens of player controllers and knows the node/scene tree, signals, resources, CharacterBody physics, and export variables inside out.
...
# TASK
Build the player controller for my game in Godot 4 (GDScript). Implement the movement I describe (platformer/top-down/first-person as applicable) with the feel-critical details: acceleration curves, coyote time and input buffering for jumps (if platforming), collision handling done the Godot 4 (GDScript)-correct way, and every tuning parameter exposed for iteration.
...
# OUTPUT FORMAT
- Complete player controller for Godot 4 (GDScript)
