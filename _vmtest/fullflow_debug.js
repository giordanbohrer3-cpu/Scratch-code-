...
(async () => {
  ...
  function nav(c, items, dvx, dvy, no, K) {
    ...
    const ch = T(c), dx = tx - ch.x; const g = gv(no) === 1;
    let goR = dx > 3, goL = dx < -3;
    if (ty < ch.y - 22 && g && Math.abs(dx) < 14) { goR = true; goL = false; }
    key(K.r, goR); key(K.l, goL);
    const needUp = ty > ch.y + 6, stuck = Math.abs(ch.x - (prev[c] || 0)) < 0.7 && Math.abs(dx) > 4;
    key(K.u, g && (needUp || stuck)); prev[c] = ch.x; return [tx, ty];
  }
  ...
  while (Date.now() - t0 < 45000 && gv('fase') < 3) {
    if (gv('modo') === 2) {
      const tt = nav('Theo', THEO, 'dtx', 'dty', 'no_theo', KT);
      const tl = nav('Lia', LIA, 'dlx', 'dly', 'no_lia', KL);
      if (gv('fase') === 2 && i % 7 === 0) console.log(`f2 t=${((Date.now() - t0) / 1000).toFixed(1)} ` +
        `Theo(${T('Theo').x.toFixed(0)},${T('Theo').y.toFixed(0)},n${gv('no_theo')})->(${tt[0].toFixed(0)},${tt[1].toFixed(0)}) it=${gv('item1')}${gv('item2')} ok${gv('Theo_ok')} | ` +
        `Lia(${T('Lia').x.toFixed(0)},${T('Lia').y.toFixed(0)},n${gv('no_lia')})->(${tl[0].toFixed(0)},${tl[1].toFixed(0)}) it=${gv('item3')}${gv('item4')} ok${gv('Lia_ok')} R=${gv('Relíquias')}`);
      i++;
    } else { ['a', 'd', 'w', 'ArrowLeft', 'ArrowRight', 'ArrowUp'].forEach(k => key(k, false)); }
    await sleep(95);
  }
  console.log('FIM fase=', gv('fase'), 'modo=', gv('modo'));
