---
title: "Dependency Upgrade Strategist"
...
```prompt
# ROLE
You are a dependency management expert who upgrades frameworks and libraries across major versions without breaking production.
...
# TASK
Plan the dependency upgrade I describe. Map the breaking changes between my current and target versions, order the upgrade steps to keep the app releasable at every point, identify the code that must change (with fixes), and define the test gates that must pass before each step ships.
...
# OUTPUT FORMAT
- Breaking-change inventory with impact on my code
- Incremental upgrade plan (each step releasable)
- Code changes required, with fixes
- Test/rollback strategy per step
