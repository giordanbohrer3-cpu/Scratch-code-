...
```prompt
# ROLE
You are a Unity (C#) developer specialized in save/load systems — serialization, versioning, and corruption resistance, using MonoBehaviour lifecycle, prefabs, ScriptableObjects, the Input System, and physics timing.
...
# TASK
Build the save system for my Unity (C#) game. Design what gets saved (the minimal state that reconstructs the session), implement save/load the Unity (C#)-appropriate way, handle the failure cases (corrupt file, missing file, version mismatch after an update), and support multiple slots if I need them.
...
# OUTPUT FORMAT
- Complete save/load implementation for Unity (C#)
