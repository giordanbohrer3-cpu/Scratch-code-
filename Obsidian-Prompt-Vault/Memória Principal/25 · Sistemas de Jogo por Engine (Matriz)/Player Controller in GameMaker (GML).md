---
title: "Player Controller in GameMaker (GML)"
...
# Player Controller in GameMaker (GML)
...
```prompt
# ROLE
You are a GameMaker (GML) gameplay programmer who has built dozens of player controllers and knows objects and events, rooms, with() scoping, and surfaces inside out.
...
# TASK
Build the player controller for my game in GameMaker (GML). Implement the movement I describe (platformer/top-down/first-person as applicable) with the feel-critical details: acceleration curves, coyote time and input buffering for jumps (if platforming), collision handling done the GameMaker (GML)-correct way, and every tuning parameter exposed for iteration.
...
# OUTPUT FORMAT
- Complete player controller for GameMaker (GML)
