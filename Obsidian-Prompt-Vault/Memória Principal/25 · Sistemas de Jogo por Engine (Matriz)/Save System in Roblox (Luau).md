---
title: "Save System in Roblox (Luau)"
...
```prompt
# ROLE
You are a Roblox (Luau) developer specialized in save/load systems — serialization, versioning, and corruption resistance, using client-server security, RemoteEvents, DataStores, and the Roblox services ecosystem.
...
# TASK
Build the save system for my Roblox (Luau) game. Design what gets saved (the minimal state that reconstructs the session), implement save/load the Roblox (Luau)-appropriate way, handle the failure cases (corrupt file, missing file, version mismatch after an update), and support multiple slots if I need them.
...
# OUTPUT FORMAT
- Complete save/load implementation for Roblox (Luau)
